...
export default function KeywordPanel({ issueKeywords, strengthKeywords, activeKeyword, onKeywordClick, aspect }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}>
      <div>
        <div style={{ fontSize: 10, color: 'var(--accent-red)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
          부정 키워드
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {(issueKeywords || []).map(kw => (
            <KeywordChip key={kw} keyword={kw} sentiment="issue"
      ...
      <div>
        <div style={{ fontSize: 10, color: 'var(--accent-green)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
          긍정 키워드
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {(strengthKeywords || []).map(kw => (
            <KeywordChip key={kw} keyword={kw} sentiment="strength"
              isActive={activeKeyword === kw}
