...
export default function AspectPanel({ aspect, aspectData, activeKeyword, keywordSentiment, onKeywordClick }) {
  ...
  return (
    <div style={{ background: 'var(--bg-surface)', border: `1px solid ${borderColor}`, borderRadius: 12, overflow: 'hidden' }}>
      <div
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{ASPECT_LABELS[aspect]}</span>
          {urgency >= 5 && (
            <span style={{
              fontSize: 10, padding: '2px 8px', borderRadius: 10,
              background: `${us.color}22`, color: us.color,
              border: `1px solid ${us.color}44`, fontWeight: 700, letterSpacing: 0.5,
            }}>
              {us.label}
            </span>
          )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ fontSize: 13, fontWeight: 600, color: us.color }}>{urgency.toFixed(1)}</span>
      ...
      {expanded && (
        <div style={{ padding: '0 18px 18px', borderTop: '1px solid var(--border-subtle)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 14 }}>
            <div style={{ background: 'rgba(255,85,85,0.05)', borderRadius: 8, padding: 12 }}>
              <div style={{ fontSize: 10, color: 'var(--accent-red)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>이슈 요약</div>
              ...
              {hasFlag && (
                <div style={{ marginTop: 6, fontSize: 10, color: 'var(--accent-orange)' }}>
                  ⚠ {hasFlag === 'llm_summary_replaced' ? 'LLM 요약 신뢰도 낮음 (원문 대체)' : '근거 연관성 낮을 수 있음'}
                </div>
              )}
            </div>
            <div style={{ background: 'rgba(78,203,113,0.05)', borderRadius: 8, padding: 12 }}>
              <div style={{ fontSize: 10, color: 'var(--accent-green)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>강점 요약</div>
              <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.65, margin: 0 }}>
          ...
          {aspectData?.issue_summary && aspectData?.issue_summary !== '—' && aspectData?.representative_reviews?.length > 0 && (
            <div style={{ marginTop: 14 }}>
              <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>대표 리뷰</div>
          ...
          {aspectData?.improvement_points?.length > 0 && (
            <div style={{ marginTop: 14 }}>
              <div style={{ fontSize: 10, color: 'var(--accent-blue)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>개선 포인트</div>
