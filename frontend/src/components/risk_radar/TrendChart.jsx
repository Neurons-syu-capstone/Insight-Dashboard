...
export function RatingTrendChart({ data }) {
  ...
  const ticks = data.map((d) => d.date).filter((_, i) => i % 6 === 0)
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
        ...
        <Line type="monotone" dataKey="avg_rating" stroke="#6366f1" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
      </LineChart>
    </ResponsiveContainer>
  )
}
    ...
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data} margin={{ top: 8, right: 16, left: -10, bottom: 0 }}>
        ...
        <Tooltip
          ...
          labelStyle={{ fontSize: 12 }}
        />
        <Line type="monotone" dataKey="neg_ratio" stroke="#d32f2f" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
      </LineChart>
    </ResponsiveContainer>
  )
}
